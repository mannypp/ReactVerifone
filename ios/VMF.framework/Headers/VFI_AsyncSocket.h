...
@class VFI_AsyncSocket;
@class VFI_AsyncReadPacket;
@class VFI_AsyncWritePacket;

extern NSString *const VFI_AsyncSocketException;
extern NSString *const VFI_AsyncSocketErrorDomain;
...
**/
- (void)onSocket:(VFI_AsyncSocket *)sock willDisconnectWithError:(NSError *)err;
...
**/
- (void)onSocketDidDisconnect:(VFI_AsyncSocket *)sock;

/**
 * Called when a socket accepts a connection.  Another socket is spawned to handle it. The new socket will have
 * the same delegate and will call "onSocket:didConnectToHost:port:".
**/
- (void)onSocket:(VFI_AsyncSocket *)sock didAcceptNewSocket:(VFI_AsyncSocket *)newSocket;

/**
 * Called when a new socket is spawned to handle a connection.  This method should return the run-loop of the
 * thread on which the new socket and its delegate should operate. If omitted, [NSRunLoop currentRunLoop] is used.
**/
- (NSRunLoop *)onSocket:(VFI_AsyncSocket *)sock wantsRunLoopForNewSocket:(VFI_AsyncSocket *)newSocket;
...
**/
- (BOOL)onSocketWillConnect:(VFI_AsyncSocket *)sock;

/**
 * Called when a socket connects and is ready for reading and writing.
 * The host parameter will be an IP address, not a DNS name.
**/
- (void)onSocket:(VFI_AsyncSocket *)sock didConnectToHost:(NSString *)host port:(UInt16)port;

/**
 * Called when a socket has completed reading the requested data into memory.
 * Not called if there is an error.
**/
- (void)onSocket:(VFI_AsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)onSocket:(VFI_AsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)onSocket:(VFI_AsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)onSocket:(VFI_AsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)onSocket:(VFI_AsyncSocket *)sock
...
**/
- (void)onSocketDidSecure:(VFI_AsyncSocket *)sock;
...
@interface VFI_AsyncSocket : NSObject
{
	...
	VFI_AsyncReadPacket *theCurrentRead;
	NSTimer *theReadTimer;
	NSMutableData *partialReadBuffer;
	
	NSMutableArray *theWriteQueue;
	VFI_AsyncWritePacket *theCurrentWrite;
